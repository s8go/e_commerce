...
let images = product.querySelectorAll("img");
...
function productView() {
  ...
  if (window.screen.width < 1025) {
    images.forEach((image, index) => {
      image.style.transform = `translateX(${100 * (index - productImage)}%)`;
    });
  } else {
  ...
  product.style.backgroundImage = `url("image-product-${
    productImage + 1
  }.jpg")`;
...
function imgSlideRight() {
  if (productImage < images.length - 1) productImage++;
  else productImage = 0;
  productView();
}

function imgSlideLeft() {
  if (productImage == 0) productImage = images.length - 1;
  else productImage--;
  productView();
}
...
addBtn.addEventListener("click", function () {
  if (Number(num.innerText) !== 0) {
    ...
    totalPrice.innerText = `$${
      Number(productPrice) * Number(numSpan.innerText)
    }.00`;
  } else {
    cartH3.innerText = num.innerText;
...
let desktopImages = document
  .querySelector(".prod-images")
  .querySelectorAll("img");
let imgSpan = document
  .querySelector(".prod-images")
  .querySelectorAll(".img-span");
...
let desktopImages2 = viewbox
  .querySelector(".prod-images")
  .querySelectorAll("img");
let imgSpan2 = viewbox
  .querySelector(".prod-images")
  .querySelectorAll(".img-span");
